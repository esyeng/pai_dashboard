...
const ThreadList: React.FC = () => {
    ...
    const handleThreadNameKeyDown = async (event: React.KeyboardEvent<HTMLSpanElement>, thread: Thread) => {
        if (event.key === 'Enter') {
        ...
        <div className=" rounded-xl shadow-xl">
            ...
            <ul className="space-y-2">
                {threadsDummyData.map((thread) => (
                    ...
                    >
                        <span className="text-gray-700 font-bold" contentEditable="true" onInput={handleThreadNameInput
                        }
                            onKeyDown={(event) => handleThreadNameKeyDown(event, thread)}
                        >
                            {thread.title}
                        </span>
                        <button
                ...
                <div className="rounded-tl-md rounded-tr-md ">
                    <ul className="flex  items-stretch justify-stretch">
                        ...
                        <li className=" w-100 flex-1 rounded-tl-md rounded-tr-md transition-colors duration-300 hover:bg-mint border-mint border-b-1 border-l-1">
                            <button className="py-2 px-4 border-mint border-b-1">Tab 2</button>
                        </li>
                    </ul>
                </div>
                {/* Tab Content */}
